...
export function setupCamera(app: PIXI.Application, viewport: PIXI.Container) {
  ...
  window.addEventListener('keydown', (e: KeyboardEvent) => {
    switch (e.key) {
      case 'ArrowUp':    viewport.position.y += panSpeed; break;
      case 'ArrowDown':  viewport.position.y -= panSpeed; break;
      case 'ArrowLeft':  viewport.position.x += panSpeed; break;
      case 'ArrowRight': viewport.position.x -= panSpeed; break;
      case '+':
      case '=':
        viewport.scale.set(
